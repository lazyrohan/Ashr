...
void CWorldCreature::Move(Point2 targetPos, double speed, double time)
{
...
Point2 CWorldCreature::GetCurPos()
{
	return curPos;
}

string CWorldCreature::RandomName()
{
...
double CWorldCreature::GrowRatio(unsigned time)
{
...
double CWorldCreature::MoveRatio(unsigned time)
{
	if ((time >= 18 && time <= 24) || (time >= 0 && time <= 8))
		return 0.5f;
	else
